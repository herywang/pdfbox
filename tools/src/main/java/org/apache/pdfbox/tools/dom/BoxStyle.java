...
public class BoxStyle {
    ...
    public static final String DEFAULT_POSITION = "absolute";
    public static final String TRANSPARENT_COLOR = "rgba(0,0,0,0)";
    // 支持的Font Family
    private static String[] KNOWN_FONT_FAMILY = {"Times New Roman", "Times", "Garamond", "Helvetica", "Arial Narrow",
            "Arial", "Verdana", "Courier New", "MS Sans Serif", "DengXian"};
    ...
    public String toString() {
		...
		if (position != null && !position.equals(DEFAULT_POSITION)) {
		...
		if (fontWeight != null && !DEFAULT_FONT_WEIGHT.equals(fontWeight)) {
			appendString(ret, "font-weight", fontWeight);
		}
		if (fontStyle != null && !DEFAULT_FONT_STYLE.equals(fontStyle)) {
		...
		if (color != null && !DEFAULT_COLOR.equals(color)) {
			appendString(ret, "color", color);
		}
        if(width != 0){
            appendLength(ret, "width", width);
        }
		if (strokeColor != null && !strokeColor.equals(TRANSPARENT_COLOR)) {
    ...
    public void setFontFamily(PDFont font, FontTable fontTable){
        String fontName = font.getName();
        String finalFontFamily = "";
        for(String fontFamily: KNOWN_FONT_FAMILY){
            if(fontName.toLowerCase().lastIndexOf(fontFamily.toLowerCase().replaceAll("\\s+", "")) >= 0){
                finalFontFamily = fontFamily;
                break;
            }
        }
        if(!finalFontFamily.isEmpty()){
            this.fontFamily = finalFontFamily;
            return;
        }

        finalFontFamily = fontTable.getUsedName(font);
        if(finalFontFamily == null || finalFontFamily.isEmpty()){
...
//		String strokeCss = "-webkit-text-stroke: %color% 1px ;" +
        String strokeCss = "-webkit-text-stroke: %color%;" +
                "text-shadow:" +
                "-1px -1px 0 %color%, " +
                "1px -1px 0 %color%," +
                "-1px 1px 0 %color%, " +
        ...
        return strokeCss.replace("%color%", color);
    }
    ...
    public void setFontFamily(String fontFamily) {
        if(fontFamily.contains("宋体")){
            this.fontFamily = "SimSun, '宋体', STSong, sans-serif";
        }else {
...
//        else if (!fontWeight.equals(other.fontWeight)) return false;
		if (Float.floatToIntBits(letterSpacing) != Float
				.floatToIntBits(other.letterSpacing)) {
			return false;
		}
		if (Float.floatToIntBits(wordSpacing) != Float
				.floatToIntBits(other.wordSpacing)) {
			return false;
		}
        return true;
    }
